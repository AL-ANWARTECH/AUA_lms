...
class Quiz(models.Model):
    ...
    title = models.CharField(max_length=200)
    description = models.TextField(blank=True)
    time_limit = models.IntegerField(help_text=_("Time limit in minutes"), null=True, blank=True)
    max_attempts = models.IntegerField(default=1, help_text=_("Maximum number of attempts allowed"))
    passing_score = models.FloatField(default=70.0, help_text=_("Minimum score percentage to pass"))
...
class QuizAttempt(models.Model):
    ...
    time_taken = models.IntegerField(help_text=_("Time taken in seconds"), null=True, blank=True)
...
class Forum(models.Model):
    ...
    title = models.CharField(max_length=200, default=_('Course Forum'))
    description = models.TextField(blank=True)
    is_active = models.BooleanField(default=True)
...
class CertificateTemplate(models.Model):
    ...
    description = models.TextField(default=_("This is to certify that the student has successfully completed the course."))
    ...
    def __str__(self):
        course_name = self.course.title if self.course else _("Global Template")
...
class Notification(models.Model):
    NOTIFICATION_TYPES = [
        ...
        ('forum_post', _('Forum Post')),
        ('assignment_due', _('Assignment Due')),
        ('certificate_earned', _('Certificate Earned')),
...
class Analytics(models.Model):
    ...
    ANALYTICS_TYPES = [
        ...
        ('quiz_attempt', _('Quiz Attempt')),
        ('assignment_submission', _('Assignment Submission')),
        ('forum_activity', _('Forum Activity')),
...
class Report(models.Model):
    ...
    REPORT_TYPES = [
        ('course_performance', _('Course Performance')),
        ('student_progress', _('Student Progress')),
        ('user_engagement', _('User Engagement')),
        ('system_usage', _('System Usage')),
        ('grade_distribution', _('Grade Distribution')),
        ('certificate_issuance', _('Certificate Issuance')),
    ]
...
class DashboardWidget(models.Model):
    ...
    WIDGET_TYPES = [
        ('enrollment_chart', _('Enrollment Chart')),
        ('progress_chart', _('Progress Chart')),
        ('grade_distribution', _('Grade Distribution')),
        ('recent_activities', _('Recent Activities')),
        ('user_statistics', _('User Statistics')),
        ('course_statistics', _('Course Statistics')),
    ]
...
class AccessibilitySettings(models.Model):
    ...
    audio_volume_level = models.IntegerField(default=50, help_text=_("Default volume level (0-100)"))
...
class AccessibilityAudit(models.Model):
    ...
    AUDIT_TYPES = [
        ('automated', _('Automated Scan')),
        ('manual', _('Manual Review')),
        ('user_feedback', _('User Feedback')),
    ...
    def __str__(self):
        return f"{self.audit_type} - {self.severity} - {'Resolved' if self.resolved else _('Pending')}"

class ScreenReaderContent(models.Model):
    """Alternative content for screen readers"""
    page_section = models.CharField(max_length=100, help_text=_("Name of the page section"))
    content_type = models.CharField(max_length=50, choices=[
        ('navigation', _('Navigation')),
        ('form', _('Form')),
        ('media', _('Media')),
        ('data_table', _('Data Table')),
        ('chart', _('Chart')),
        ('other', _('Other')),
    ])
    alternative_text = models.TextField(help_text=_("Descriptive text for screen readers"))
    is_active = models.BooleanField(default=True)
    created_at = models.DateTimeField(auto_now_add=True)
...
class KeyboardShortcut(models.Model):
    ...
    ACTION_CHOICES = [
        ('navigate_home', _('Navigate to Home')),
        ('open_menu', _('Open Menu')),
        ('search_focus', _('Focus Search')),
        ('skip_to_content', _('Skip to Content')),
        ('toggle_sidebar', _('Toggle Sidebar')),
        ('previous_item', _('Previous Item')),
        ('next_item', _('Next Item')),
        ('select_item', _('Select Item')),
        ('close_modal', _('Close Modal')),
        ('save_changes', _('Save Changes')),
        ('cancel_action', _('Cancel Action')),
    ]
    
    key_combination = models.CharField(max_length=20, help_text=_("e.g., Ctrl+S, Alt+F, Tab"))
    action = models.CharField(max_length=20, choices=ACTION_CHOICES)
    description = models.TextField()
    is_global = models.BooleanField(default=True, help_text=_("Available on all pages"))
    is_active = models.BooleanField(default=True)
